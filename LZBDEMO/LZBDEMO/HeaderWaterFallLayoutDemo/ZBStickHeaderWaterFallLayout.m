//
//  ZBStickHeaderWaterFallLayout.m
//  LZBDEMO
//
...
static NSString *const ZBStickHeaderWaterCellKind = @"WaterFallCell";           //cell
static NSString *const ZBStickHeaderWaterDecorationKind = @"decoration";        //装饰

#import "ZBStickHeaderWaterFallLayout.h"

@interface ZBStickHeaderWaterFallLayout ()

@property(strong ,nonatomic)NSArray *topInsetArray;             //每个section的top
@property(strong ,nonatomic)NSArray *bottomInsetArray;          //每个section的bottom
@property(strong ,nonatomic)NSArray *itemInnerMarginArray;      //item的外边距
@property(strong ,nonatomic)NSArray *columnsCountArray;         //每个section的columnCount
@property(strong ,nonatomic)NSArray *itemsWidthArray;           //每个item的width
@property(strong ,nonatomic)NSArray *headerToTopArray;          //每个section停留时与顶部的距离
@property(strong ,nonatomic)NSDictionary *layoutInfo;           //全部layoutAttribute的信息
@property(strong ,nonatomic)NSArray *sectionsHeights;
@property(strong ,nonatomic)NSArray *itemsInSectionsHeights;    //每个section里cell的高度

@end

@implementation ZBStickHeaderWaterFallLayout
...
-(void)setup {
    ...
    self.headAlignment = ZBStickHeaderAlignmentLeft;
...
-(void)prepareLayout {
    ...
    if (self.collectionView.isDecelerating || self.collectionView.isDragging) {
        ...
        //计算每个section的停留时与顶部的距离,通过代理方法（collectionView:layout:headerToTopInSection:）获得
        [self calculateHeaderToTop];
        
        //计算所在section的每个item的width（一个section只有一个width）.
        //通过代理方法（collectionView:layout:widthForItemInSection:）获得
        [self calculateItemsWidth];
        
        //计算所在section与上一个section的间距和所在section与下一个section的间距.
        //通过代理方法（collectionView:layout:topInSection:和collectionView:layout:bottomInSection:）获得
        [self calculateSectionsTopInsetAndBottomInset];

        //计算每个section的列数（根据itemsWidthArray计算列数，而itemsWidthArray是[self calculateItemsWidth]计算出来的）
        [self calculateColumnsCount];
        
        //计算每个section中item之间的间距(上下左右的间距都是同一个数值)(根据itemsWidthArray计算列数)
        [self calculateItemsInnerMarginArray];
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return self.layoutInfo[ZBStickHeaderWaterCellKind][indexPath];
...
/****************/
//计算所在section的每个item的width（一个section只有一个width）.通过代理方法（collectionView:layout:widthForItemInSection:）获得
...
- (void) calculateItemsAttributes {
    ...
    newLayoutInfo[ZBStickHeaderWaterCellKind] = cellLayoutInfo;
